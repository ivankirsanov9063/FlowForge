...
namespace NetConfig {
    ...
    struct CidrV4 { std::uint32_t addr_be; std::uint8_t prefix; }; // addr in network byte order
    struct CidrV6 { std::array<std::uint8_t,16> addr; std::uint8_t prefix; };

    struct Params {
        int mtu = 1400;
        // адреса как в твоём скрипте
        CidrV4 v4_local { inet_addr("10.8.0.1"), 32 };
        std::uint32_t v4_peer_be = inet_addr("10.8.0.2");

        CidrV6 v6_local { {0xfd,0x00,0xde,0xad,0xbe,0xef,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01}, 128 };
        std::array<std::uint8_t,16> v6_peer { {0xfd,0x00,0xde,0xad,0xbe,0xef,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x02} };
    ...
    inline bool write_sysctl(const char* path, const char* val) {
    ...
    inline nl_sock* nl_connect_route() {
        ...
        if (nl_connect(sk, NETLINK_ROUTE) < 0) { nl_socket_free(sk); return nullptr; }
        return sk;
    }

    inline bool link_set_up_and_mtu(nl_sock* sk, int ifindex, int mtu) {
    ...
    inline bool addr_flush_all(nl_sock* sk, int ifindex) {
        nl_cache* cache = nullptr;
        if (rtnl_addr_alloc_cache(sk, &cache) < 0) return false;

        std::vector<rtnl_addr*> to_del;
        for (nl_object* it = nl_cache_get_first(cache); it; it = nl_cache_get_next(it)) {
    ...
    inline bool addr_add_v4_p2p(nl_sock* sk, int ifindex,
                                std::uint32_t local_be, std::uint32_t peer_be,
                                std::uint8_t prefix) {
        ...
        rtnl_addr_set_peer(a,  p); // peer для IPv4 поддерживается ядром
        rtnl_addr_set_prefixlen(a, prefix);

        const int rc = rtnl_addr_add(sk, a, 0);
        nl_addr_put(l); nl_addr_put(p); rtnl_addr_put(a);
        return rc == 0 || rc == -NLE_EXIST;
    }

    inline bool addr_add_v6_local(nl_sock* sk, int ifindex,
        ...
        if (!l) { rtnl_addr_put(a); return false; }
    ...
    inline bool route_add_onlink_host_v6(nl_sock* sk, int ifindex,
        ...
        if (!d) { rtnl_route_put(r); return false; }
        nl_addr_set_prefixlen(d, 128);
        rtnl_route_set_dst(r, d);

        // nexthop с oif
        rtnl_nexthop* nh = rtnl_route_nh_alloc();
        if (!nh) { nl_addr_put(d); rtnl_route_put(r); return false; }
        rtnl_route_nh_set_ifindex(nh, ifindex);
        rtnl_route_add_nexthop(r, nh);

        const int rc = rtnl_route_add(sk, r, 0);
        nl_addr_put(d);
        // nh освобождается вместе с r
        rtnl_route_put(r);
        return rc == 0 || rc == -NLE_EXIST;
    }

    inline std::optional<std::string> find_default_oifname(nl_sock* sk, int family) {
        nl_cache* rcache = nullptr;
        nl_cache* lcache = nullptr;
        // В libnl 3.7 эта сигнатура допустима
        if (rtnl_route_alloc_cache(sk, family, 0, &rcache) < 0) return std::nullopt;
        if (rtnl_link_alloc_cache(sk, AF_UNSPEC, &lcache) < 0) { nl_cache_free(rcache); return std::nullopt; }

        int oif = 0;
        for (nl_object* it = nl_cache_get_first(rcache); it; it = nl_cache_get_next(it)) {
            ...
            // В 3.7 не используем rtnl_route_get_oif*, берём из nexthop
        ...
        if (oif > 0) {
            ...
            if (link) { name = rtnl_link_get_name(link); rtnl_link_put(link); }
        }
    ...
    inline bool ensure_nat44(const std::string& oifname, const std::string& src_cidr) {
        std::string cmd;
        cmd  = "add table ip nat\n";
        cmd += "add chain ip nat POSTROUTING { type nat hook postrouting priority 100 ; }\n";
        cmd += "add rule ip nat POSTROUTING oifname \"" + oifname + "\" ip saddr " + src_cidr + " counter masquerade\n";
        return nft_apply(cmd);
    }

    inline bool ensure_nat66(const std::string& oifname, const std::string& src_cidr) {
        std::string cmd;
        cmd  = "add table ip6 nat\n";
        cmd += "add chain ip6 nat POSTROUTING { type nat hook postrouting priority 100 ; }\n";
        cmd += "add rule ip6 nat POSTROUTING oifname \"" + oifname + "\" ip6 saddr " + src_cidr + " counter masquerade\n";
        return nft_apply(cmd);
    }

// Аналог server_network.sh
    inline bool ApplyServerSide(const std::string& ifname, const Params& p = Params{}) {
        // root/CAP_NET_ADMIN обязателен
        ...
        // 3) IPv4 p2p
        ok &= addr_add_v4_p2p(sk, ifindex, p.v4_local.addr_be, p.v4_peer_be, p.v4_local.prefix);

        // 4) IPv6 addr + маршрут на peer /128 on-link (не критично)
        ok &= addr_add_v6_local(sk, ifindex, p.v6_local.addr, p.v6_local.prefix);
        ...
        // 6) найти WAN по дефолтным маршрутам и повесить NAT
