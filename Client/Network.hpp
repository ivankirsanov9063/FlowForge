...
// --- ioctl helpers for link up / mtu (проще, чем через libnl) ---
static int if_set_up(const std::string& ifname) {
    int s = ::socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0);
    if (s < 0) return -errno;
    ifreq ifr{};
    std::snprintf(ifr.ifr_name, IFNAMSIZ, "%s", ifname.c_str());
    if (ioctl(s, SIOCGIFFLAGS, &ifr) < 0) { int e=-errno; ::close(s); return e; }
    ifr.ifr_flags |= IFF_UP;
    if (ioctl(s, SIOCSIFFLAGS, &ifr) < 0) { int e=-errno; ::close(s); return e; }
    ::close(s);
    return 0;
}
...
static int if_set_mtu(const std::string& ifname, int mtu) {
    ...
    std::snprintf(ifr.ifr_name, IFNAMSIZ, "%s", ifname.c_str());
    ifr.ifr_mtu = mtu;
    if (ioctl(s, SIOCSIFMTU, &ifr) < 0) { int e=-errno; ::close(s); return e; }
    ::close(s);
    return 0;
}
...
// --- get current default metric (table=main) ---
static std::optional<int> get_default_metric(struct nl_sock* sk, int family) {
    ...
    for (auto* obj = nl_cache_get_first(rcache); obj; obj = nl_cache_get_next(obj)) {
        ...
        if (dst && nl_addr_get_prefixlen(dst) == 0) {
            ...
            if (prio < 0) prio = 100; // разумный дефолт, если не задан
            metric = prio;
            break;
        }
    }
...
static void replace_default_via_dev(struct nl_sock* sk, int family, int oif) {
    ...
    // выбрать метрику: чуть меньше текущей, иначе 5
    int metric = 5;
    ...
    // scope: v4 — LINK, v6 — UNIVERSE (как у iproute2)
    if (family == AF_INET) rtnl_route_set_scope(route, RT_SCOPE_LINK);
    else                   rtnl_route_set_scope(route, RT_SCOPE_UNIVERSE);
    ...
    // если уже есть наш default dev — обновим метрику
    err = rtnl_route_add(sk, route, NLM_F_CREATE | NLM_F_REPLACE);
