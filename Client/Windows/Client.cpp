...
static std::wstring utf8_to_wide(const std::string& s) {
    ...
    if (len > 1) MultiByteToWideChar(CP_UTF8, 0, s.c_str(), -1, ws.data(), len);
    return ws;
}

static void debug_packet_info(const std::uint8_t* data, std::size_t len, const char* direction) {
    if (len < 20) return;

    uint8_t version = (data[0] >> 4) & 0x0f;
    if (version == 4) {
        uint32_t src = (data[12] << 24) | (data[13] << 16) | (data[14] << 8) | data[15];
        uint32_t dst = (data[16] << 24) | (data[17] << 16) | (data[18] << 8) | data[19];
        std::printf("[%s] IPv4: %u.%u.%u.%u -> %u.%u.%u.%u (len=%zu)\n",
                   direction,
...
int main(int argc, char **argv) {
    ...
    if (!Wintun.load()) { std::cerr << "Failed to load wintun.dll\n"; return 1; }
    ...
    auto receive_from_net = [sess](std::uint8_t* buffer, std::size_t size) -> ssize_t {
        ...
        debug_packet_info(pkt, pktSize, "FROM_NET");

        if (pktSize > size) { Wintun.RecvRelease(sess, pkt); return -1; }
