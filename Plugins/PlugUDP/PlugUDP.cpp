...
namespace iphelpers
{
    inline bool ExtractIPv4Src(const std::uint8_t* p, std::size_t n, std::uint32_t& out_host) noexcept {
        if (n < 20 || (p[0] >> 4) != 4) return false;
        std::size_t ihl = (p[0] & 0x0Fu) * 4u; if (ihl < 20 || n < ihl) return false;
        std::uint32_t src; std::memcpy(&src, p + 12, 4); out_host = ntohl(src); return true;
    }
    inline bool ExtractIPv4Dst(const std::uint8_t* p, std::size_t n, std::uint32_t& out_host) noexcept {
        if (n < 20 || (p[0] >> 4) != 4) return false;
        std::size_t ihl = (p[0] & 0x0Fu) * 4u; if (ihl < 20 || n < ihl) return false;
        std::uint32_t dst; std::memcpy(&dst, p + 16, 4); out_host = ntohl(dst); return true;
    }

    inline bool ExtractIPv6Src(const std::uint8_t* p, std::size_t n, std::array<std::uint8_t,16>& out) noexcept {
        if (n < 40 || ((p[0] >> 4) != 6)) return false;
        std::memcpy(out.data(), p + 8, 16);  // IPv6 source offset
        return true;
    }
    inline bool ExtractIPv6Dst(const std::uint8_t* p, std::size_t n, std::array<std::uint8_t,16>& out) noexcept {
        if (n < 40 || ((p[0] >> 4) != 6)) return false;
        std::memcpy(out.data(), p + 24, 16); // IPv6 dest offset
        return true;
    }
}
...
namespace ClientSide
{
    ...
    std::mutex m_rx;
    std::deque<std::string> q_rx;                 // из сервера -> в сеть

    std::mutex m_tx;
    std::deque<std::string> q_tx;                 // из сети -> на сервер
    ...
    inline void KickSendLocked()
    {
        ...
        sock->async_send(
                ...
                [buf](const boost::system::error_code& ec, std::size_t /*n*/) {
                    ...
                    tx_in_flight = false;
                    if (!ec && !q_tx.empty()) KickSendLocked();
                }
        );
    }
    ...
    {
        std::lock_guard<std::mutex> lk(m_tx);
        q_tx.emplace_back(reinterpret_cast<const char*>(p), reinterpret_cast<const char*>(p) + n);
        KickSendLocked();
    }
    ...
    inline bool Connect(const std::string& ip, std::uint16_t port) noexcept
    {
        try {
            ...
            if (addr.is_v6()) {
                sock->open(udp::v6(), ec);
                if (ec) { StopIo(); return false; }
                // разрешим v4-mapped при необходимости
            ...
            for (unsigned i = 0; i < IoThreads; ++i) threads.emplace_back([&]{ io->run(); });
            return true;
        } catch (...) { StopIo(); return false; }
    }
}
...
namespace ServerSide
{
    ...
    struct Client
    {
        ...
        std::deque<std::string> q_tx; // пакеты из сети -> клиент
        bool tx_in_flight = false;
        std::mutex mtx;
    };

    // Унифицированный ключ VIP (v4/v6)
    struct VipKey {
        bool v6 = false;
        ...
        static VipKey FromV6(const std::array<std::uint8_t,16>& a) { VipKey k; k.v6=true; k.v6addr=a; return k; }
    ...
    std::mutex m_from;
    std::deque<std::string> from_clients; // пакеты из клиентов -> в сеть

    std::mutex m_clients;
    std::unordered_map<VipKey, std::unique_ptr<Client>, VipKey::Hasher> vip_to_client;
    ...
    inline void ContinueSend(VipKey vip)
    {
        ...
        udp::endpoint ep;
        {
            ...
            c.q_tx.pop_front();
            ep = c.ep; // копируем endpoint под лок
        }
        ...
        sock->async_send_to(
                ...
                [buf, vip](const boost::system::error_code& ec, std::size_t /*n*/) {
    ...
    inline bool Bind(std::uint16_t port) noexcept
    {
        try {
            ...
            sock.reset(); gc_timer.reset(); io.reset();
            return false;
        }
    }
                 ...
                 const volatile sig_atomic_t* working_flag) noexcept
{
    ...
    while (*working_flag)
    {
        // Net -> Server (наполняем очередь, асинхронная отправка сама по себе)
        for (;;)
        {
            ssize_t got = receive_from_net(buf.data(), buf.size());
            if (got > 0) { ClientSide::EnqueueTx(buf.data(), static_cast<std::size_t>(got)); continue; }
            if (got == 0) break; else return 1;
        }

        // Server -> Net (считываем, что уже приняли io-потоки)
        for (;;)
        {
            std::string pkt;
            {
            ...
            if (!pkt.empty())
            {
                (void)send_to_net(reinterpret_cast<const std::uint8_t*>(pkt.data()),
                 ...
                 const volatile sig_atomic_t* working_flag) noexcept
{
    ...
    while (*working_flag)
    {
        // 1) Отдать в сеть всё, что прислали клиенты (async receive → очередь)
        for (;;)
        {
            std::string pkt;
        ...
        // 2) Прочитать из сети и разложить по клиентам (маршрутизация по dst)
        for (;;)
        {
            ssize_t got = receive_from_net(buf.data(), buf.size());
            if (got > 0) { ServerSide::EnqueueToClient(buf.data(), static_cast<std::size_t>(got)); continue; }
            if (got == 0) break; else { ServerSide::Stop(); return 1; }
        }
    }
